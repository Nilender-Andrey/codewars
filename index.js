...
function formatDuration(seconds) {
  ...
  const tameArr = parseDate(seconds).reduce((str, item, index, arr) => {
    if (item === 0) return str;

    const notZero = arr.slice(index + 1).find((i) => i != 0);

    str && !notZero ? (str += ' and ') : str && (str += ', ');

    str += item === 1 ? `${item} ${time[index]}` : `${item} ${time[index]}s`;
...
function parseDate(seconds) {
  return [31536000, 86400, 3600, 60, 1].map((item) => {
    const wholePart = Math.trunc(seconds / item);
    seconds -= wholePart * item;
    return wholePart;
  });
}
 */
...
//console.log(formatDuration(132030240)); // '1 hour, 1 minute and 2 seconds';
